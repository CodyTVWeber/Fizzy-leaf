...
Christian (texts, 2026-08): **Local Delivery** page — 30 miles of College Grove; monthly-only 12/$35, 24/$65, 48/$120 + $3 delivery; address check then signup (name, address, email, pack). **No one-time** local delivery. These prices are **not** shop Subscribe & Save ($34.40 / $63.20) and **not** one-time shop ($43 / $79).
...
2. **Local delivery:** lead-gen form, **not** Shopify checkout. Different SKUs/prices; no 48-pack variant; Christian delivers himself monthly. Radius check in the browser; signup POSTs to a **new** Formspark form (placeholder until created).
...
|-------------|--------|-----------|
| `REPLACE_FORMSPARK_DELIVERY_ID` | `delivery.html` form `action` = `https://submit-form.com/REPLACE_FORMSPARK_DELIVERY_ID` | Cody/Christian create a **new** Formspark form (do **not** reuse `vwsJT57aO`) |
...
If form action still contains `REPLACE_`, **do not POST** — show the same error-style status: “Signups aren’t connected yet. Email us via Contact.” UI still works (radius + form fields).
...
- Body: *If you live in the Middle TN area you might be close enough for me to just deliver Fizzy Leaf to you! Put your address below to see if you live within the 30 mile boundary. At the beginning of each month I will be doing deliveries to each person that signs up for a 12 pack, 24 pack, or 48 pack every month.*
- Price list: **Delivery fee $3.** 12 pack **$35**/mo · 24 pack **$65**/mo · 48 pack **$120**/mo.
- Note: **Monthly subscription only — not a one-time order.** Shop one-time / Subscribe & Save stay on `shop.html`.

**Flow**

1. Address box (street, city, state, ZIP — or one-line address) + “Check my address”.
2. Geocode via **US Census** one-line geocoder (no API key):

   `https://geocoding.geo.census.gov/geocoder/locations/onelineaddress?address=…&benchmark=4&format=json`

   Haversine miles vs `ORIGIN` `{ lat: 35.7869, lng: -86.6750 }`. Radius **30**.
3. **Outside / unknown:** message; **hide** signup.
4. **Inside:** show miles (1 decimal) + signup: name, email, address (prefill checked address), pack (`select` or radios: 12 / 24 / 48). Hidden: `miles`, `lat`, `lng`, `plan=monthly`, `delivery_fee=3`. No quantity stepper, no one-time option.
...
No npm test suite. Manual: `python3 -m http.server` → shop prices render (fallback then live one-time); subscribe still shows ~$34.40/$63.20 if plan read denied; cart add + drawer; delivery in-radius (Franklin/Spring Hill) vs out (e.g. Knoxville); form placeholder does not POST; mobile ≤768 padding; no-flash bg.
...
- Creating the Formspark delivery form (placeholder only)
...
- [ ] Signup hidden until in-radius; monthly packs only
- [ ] Placeholder Formspark: submit shows not-connected message, no network POST
